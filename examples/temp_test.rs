...
#[derive(
    Serialize, Deserialize, PartialEq, Debug, Eq, PartialOrd, Ord,
)]
struct SKey {
    s: String,
}
...
fn main() {
    ...
    thing
        .insert(SKey { s: "a".to_owned() }, SKey { s: "b".to_owned() });
    println!(
        "test_map_key_value: {:?}",
        serde_yml::to_string(&thing).unwrap()
    );

    let thing = S { s: " ".to_owned() };
    println!(
        "test_moar_strings_needing_quote: {:?}",
        serde_yml::to_string(&thing).unwrap()
    );
